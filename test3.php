...
<head>
    ...
    <style>
        ...
        .sub-title-pro { font-size: 13px; color: var(--main); font-weight: 700; margin-top: 10px; letter-spacing: 5px; opacity: 0; animation: fadeInPro 1s 1.2s forwards; font-family: 'Poppins', sans-serif; }
        ...
        @keyframes fadeInPro { to { opacity: 1; letter-spacing: 2px; } }
        ...
        .side-glow { position: fixed; top: 0; right: 0; width: 50%; height: 100%; background: radial-gradient(circle at right, rgba(13, 45, 68, 0.6) 0%, transparent 70%); z-index: -1; }
        ...
        .play-btn-premium { 
            width: 90%; margin: 20px auto; display: flex; justify-content: center; align-items: center; gap: 10px;
        ...
        .play-btn-premium:hover { background: var(--main); box-shadow: 0 8px 30px rgba(225, 29, 72, 0.8); transform: translateY(-3px); }
...
function play(id, src) {
    ...
    // إذا كانت القناة 10 أو 11 (ستارز بلاي)، نطلب الرابط من السيرفر كبث مباشر
    if (src === 'b10.php' || src === 'b11.php') {
        finalUrl = window.location.origin + '/' + src;
        runHls(video, finalUrl);
    } else {
        // قنوات beIN تجلب الرابط المكتوب داخل الملف
        fetch(src)
            .then(response => response.text())
            .then(url => runHls(video, url.trim()))
            .catch(err => console.error("خطأ في جلب الرابط"));
    }
}
