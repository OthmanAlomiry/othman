...
<head>
    ...
    <style>
        ...
        .match-slider { display: flex; gap: 8px; overflow-x: auto; padding: 5px 15px; scrollbar-width: none; background: rgba(255,255,255,0.02); -webkit-overflow-scrolling: touch; }
        .match-slider::-webkit-scrollbar { display: none; }
        .mini-card { min-width: 125px; background: var(--glass); border: 1px solid var(--glass-border); border-radius: 12px; padding: 6px; text-align: center; flex-shrink: 0; }
        ...
        .mini-league { font-size: 7px; opacity: 0.4; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 2px; }

        /* ستايلات عثمان الأساسية */
        ...
        .video-box { width: 100%; aspect-ratio: 16/9; background: #000; background-size: cover; background-position: center; position: relative; }
...
<div class="main-container">
    <div id="notify-panel" style="position:fixed; bottom:85px; left:50%; transform:translateX(-50%); width:280px; max-height:350px; background:rgba(15,23,42,0.95); backdrop-filter:blur(15px); border-radius:20px; border:1px solid var(--glass-border); z-index:5500; display:none; flex-direction:column; overflow:hidden;">
        <div style="background:var(--main); color:white; padding:12px; font-weight:900; font-size:13px; display:flex; justify-content:space-between;"><span>中心 الإشعارات</span><i class="fas fa-times" onclick="toggleNotifyPanel()" style="cursor:pointer;"></i></div>
    ...
    <div class="visitors-badge-float"><i class="fas fa-users" style="color:#22c55e; font-size:14px;"></i><span id="realtime-visitors" style="font-size:10px; font-weight:900; color:#fff;"><?php echo $online_now; ?></span></div>
    ...
    <div class="header-fixed">
        ...
        <div style="background:rgba(225,29,72,0.15); height:32px; overflow:hidden; display:flex; align-items:center; margin-bottom:10px; position:relative; border-bottom:1px solid rgba(255,255,255,0.05);">
            <span style="background:var(--main); color:#fff; padding:0 12px; height:100%; display:flex; align-items:center; font-size:10px; font-weight:900; z-index:10; position:absolute; right:0;">تنبيهات</span>
            <div style="flex:1; overflow:hidden; direction:ltr;"><marquee style="color:#fff; font-size:12px; font-weight:700;" scrollamount="5"><?= $news['text'] ?></marquee></div>
        </div>
        <?php endif; ?>
        ...
        <div class="match-slider">
            ...
            if($f_count == 0) echo '<p style="font-size:10px; opacity:0.3; width:100%; text-align:center;">لا توجد مباريات هامة اليوم</p>';
            ?>
        </div>
...
<script>
function toggleNotifyPanel() { let panel = document.getElementById('notify-panel'); panel.style.display = (panel.style.display === 'flex') ? 'none' : 'flex'; }
