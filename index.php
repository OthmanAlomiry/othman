...
if(isset($_GET['fetch_schedule'])) {
    ...
    if(empty($grouped)) {
        echo "<p style='text-align:center; padding:50px; opacity:0.5; font-size:12px; color:#fff;'>لا توجد مباريات هامة لهذا التاريخ</p>";
    } else {
...
<head>
    ...
    <style>
        ...
        .date-nav { display: flex; align-items: center; justify-content: space-between; padding: 12px; background: rgba(255,255,255,0.05); border-bottom: 1px solid var(--glass-border); color: #fff; }
        .date-nav b { font-size: 13px; color: #0ea5e9; }
        .date-nav button { background: var(--main); border: none; color: #fff; width: 30px; height: 30px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; }
...
function loadSchedule(date) {
    ...
    content.innerHTML = '<p style="text-align:center; padding:50px; opacity:0.5; color:#fff;">جاري التحميل...</p>';
