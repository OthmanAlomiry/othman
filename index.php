...
$cache_file = "matches_v3_cache.json"; 
$cache_time = 900; // تحديث كل 15 دقيقة فقط
...
$important_leagues = [307, 2, 3, 5, 39, 140, 135, 78, 61, 1, 235]; 
$my_matches = array_filter($all_fixtures, function($m) use ($important_leagues) {
    return in_array($m['league']['id'], $important_leagues);
});
// إذا لم توجد مباريات هامة، اعرض أول 10 مباريات من أي دوري لضمان عدم الفراغ
...
<head>
    ...
    <style>
        ...
        /* ستايل قسم المباريات الاحترافي عثمان */
        .matches-container { padding: 0 12px; margin-bottom: 30px; }
        .match-card { background: rgba(255, 255, 255, 0.03); border: 1px solid var(--glass-border); border-radius: 18px; padding: 18px 10px; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; position: relative; transition: 0.3s; }
        .m-league { position: absolute; top: -10px; right: 15px; background: var(--sky); font-size: 9px; padding: 3px 10px; border-radius: 50px; font-weight: 900; box-shadow: 0 4px 8px rgba(14, 165, 233, 0.3); }
        ...
        .m-info { flex: 0.8; text-align: center; border-left: 1px solid rgba(255,255,255,0.05); border-right: 1px solid rgba(255,255,255,0.05); margin: 0 5px; }
        ...
        <div id="section-matches" class="channel-section active">
            <div class="matches-container">
                ...
                <?php if(empty($my_matches)): ?>
                    <div style="text-align:center; padding:40px; background:var(--glass); border-radius:20px; opacity:0.3; font-size:12px;">لا توجد مباريات مسجلة حالياً</div>
