...
if(isset($_SESSION['ok'])){
    ...
    if(isset($_POST['save_sec'])){
        ...
        $new_sec = [
            'id' => $sec_id,
            'name' => $_POST['sec_name'],
            'key' => $_POST['sec_key'], // الكود البرمجي مثل bein
            'img' => $_POST['sec_img'], // مسار الصورة mg/bein.png
            'status' => $_POST['sec_status'] // show أو hide
        ];
        ...
        callCloud("PUT", $BIN_ID, $API_KEY, ['custom_channels'=>$channels, 'sections'=>$sections]);
    ...
    if(isset($_POST['save_ch'])){
        ...
        callCloud("PUT", $BIN_ID, $API_KEY, ['custom_channels'=>array_values($channels), 'sections'=>$sections]);
...
<head>
    ...
    <title>لوحة التحكم المطورة - عثمان</title>
    ...
    <style>
        ...
        .box { max-width: 600px; margin: 10px auto; background: rgba(255,255,255,0.05); padding: 20px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); }
        h2 { color: #e11d48; font-size: 1.1rem; border-bottom: 1px solid #333; padding-bottom: 10px; }
        input, select, button { width: 100%; padding: 10px; margin: 5px 0; border-radius: 8px; border: 1px solid #333; background: #111; color: white; box-sizing: border-box; }
...
<?php else: ?>
    ...
    <div class="box" id="sections_area">
        ...
        <form method="POST">
            ...
            <input name="sec_name" placeholder="اسم القسم (مثال: باقة SSC)" value="<?= $edit_sec ? $edit_sec['name'] : '' ?>" required>
            <input name="sec_key" placeholder="الكود (مثال: ssc)" value="<?= $edit_sec ? $edit_sec['key'] : '' ?>" required>
            <input name="sec_img" placeholder="مسار الصورة (مثال: mg/ssc.png)" value="<?= $edit_sec ? $edit_sec['img'] : '' ?>" required>
        ...
        <?php foreach($sections as $s): ?>
            <div class="item-card" style="border-color: <?= $s['status']=='show'?'#22c55e':'#555' ?>">
                ...
                <div class="btns"><a href="?edit_sec=<?= $s['id'] ?>#sections_area" class="edit">تعديل</a><a href="?del_sec=<?= $s['id'] ?>" class="del">حذف</a></div>
            </div>
        <?php endforeach; ?>
    </div>
    ...
    <div class="box">
        ...
        <form method="POST">
            ...
            <input name="f" placeholder="الملف (مثال: b1.php)" value="<?= $edit_ch ? $edit_ch['file'] : '' ?>" required>
            <select name="s">
        ...
        <?php foreach(array_reverse($channels) as $c): ?>
            <div class="item-card">
                ...
                <div class="btns"><a href="?edit=<?= $c['id'] ?>" class="edit">تعديل</a><a href="?del=<?= $c['id'] ?>" class="del">حذف</a></div>
            </div>
        <?php endforeach; ?>
        <center><a href="index.php" style="color:#aaa; text-decoration:none; font-size:12px;">← العودة للموقع</a> | <a href="?out=1" style="color:#ff4d4d; text-decoration:none; font-size:12px;">خروج</a></center>
    </div>
<?php endif; ?>
</body>
</html>
