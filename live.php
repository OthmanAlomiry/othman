...
<head>
    ...
    <style>
        ...
        /* تذييل الصفحة وعداد الزيارات المطور */
        footer {
            text-align: center;
            padding: 40px 10px;
            background: #fff;
...
<div class="grid">
    ...
    <div class="card">
        ...
        <div class="sch-box" id="sch-<?php echo $i; ?>">جاري جلب الجدول...</div>
    </div>
    <?php endfor; ?>
</div>
...
<script>
// --- كود العداد الفعلي المصلح والمربوط بـ Firebase ---
const FIREBASE_URL = "https://scothman-42a6e.firebaseio.com/visits.json";

async function updateVisitCount() {
    try {
        // 1. جلب الرقم من السيرفر مع منع التخزين المؤقت
        const response = await fetch(FIREBASE_URL + '?nocache=' + new Date().getTime());
        const data = await response.json();
        
        // 2. التحقق من البيانات (لو null نعتبرها 0)
        let currentCount = (data !== null && !isNaN(data)) ? parseInt(data) : 0;
        let newCount = currentCount + 1;
        
        // 3. التحديث في السيرفر فوراً
        const updateRes = await fetch(FIREBASE_URL, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(newCount)
        });

        // 4. عرض الرقم الجديد في الموقع فقط إذا نجح التحديث
        if (updateRes.ok) {
            document.getElementById('count-num').innerText = newCount.toLocaleString();
        } else {
            document.getElementById('count-num').innerText = currentCount.toLocaleString();
        }
