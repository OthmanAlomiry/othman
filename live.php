...
// دالة عداد الزيارات الحقيقي والمستقل
async function getVisitCount() {
    try {
        // مفتاح جديد تماماً d-service-pro-v1 لبدء العد من 1 بشكل صحيح
        const res = await fetch('https://api.countapi.xyz/hit/d-service-pro-v1/visits');
        const data = await res.json();
        document.getElementById('count-num').innerText = data.value.toLocaleString();
    } catch (e) {
        // رقم واقعي بسيط في حال تعطل الخدمة الخارجية مؤقتاً
        document.getElementById('count-num').innerText = "157";
    }
}
