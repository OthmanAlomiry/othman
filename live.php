...
<head>
    ...
    <style>
        ...
        body { 
            ...
            background: var(--bg); 
            padding-top: 180px; /* مساحة للقسم الثابت في الأعلى */
        }
        
        /* القسم العلوي الثابت (Sticky) */
        ...
        .promo-text strong { color: var(--main); font-weight: 700; }
        ...
        .play-btn { width: 90%; margin: 15px auto; display: block; background: var(--main); color: #fff; border: none; padding: 12px; border-radius: 10px; font-weight: bold; cursor: pointer; font-family: 'Tajawal', sans-serif; transition: 0.2s; }
        .play-btn:hover { background: #be123c; }
        
        .sch-box { padding: 12px; border-top: 1px solid #eee; min-height: 60px; font-size: 13px; color: #444; }

        /* تذييل الصفحة وعداد الزيارات */
        footer {
            text-align: center;
            padding: 40px 10px;
            background: #fff;
        ...
        .visitor-counter p { margin: 0 0 10px 0; font-size: 12px; color: #94a3b8; letter-spacing: 1px; }
        #count { font-size: 28px; font-weight: bold; color: #22c55e; font-family: monospace; text-shadow: 0 0 10px rgba(34, 197, 94, 0.4); }
...
<div class="grid">
    ...
    <div class="card">
        <div class="c-head">
            ...
            <span style="color: #22c55e; font-size: 12px;"><i class="fas fa-circle" style="font-size: 8px; vertical-align: middle;"></i> مباشر</span>
        </div>
...
// دالة عداد الزيارات الحقيقي
async function updateVisitorCount() {
    try {
        // نستخدم api.countapi.xyz لضمان بقاء الرقم حقيقي ودائم
        const response = await fetch('https://api.countapi.xyz/hit/othman-ifak-sports/visits');
        const data = await response.json();
        document.getElementById('count').innerText = data.value.toLocaleString();
    } catch (e) {
        document.getElementById('count').innerText = "متصل";
    }
}

// دالة جلب المباريات تلقائياً
        ...
        for (let i = 1; i <= 9; i++) {
            ...
            if (m) {
                container.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <span style="color:#e11d48; font-weight:bold; font-size:10px; border:1px solid #e11d48; padding:1px 5px; border-radius:4px;">LIVE</span>
                </div>`;
            } else {
                container.innerHTML = "لا توجد مباراة كبرى حالياً";
            }
        }
    } catch (e) { 
...
// تشغيل العداد والمباريات عند فتح الصفحة
