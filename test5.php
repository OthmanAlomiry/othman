...
/**
 * دالة التشغيل الذكية المحدثة لضمان عمل كافة القنوات
 */
function smartPlay(videoId, primary, backup, btn) {
    const video = document.getElementById(videoId);
    const originalText = btn.innerHTML;
    
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> <span>جاري بدء البث...</span>';
    btn.style.pointerEvents = 'none';

    // تنظيف أي نسخة قديمة من المشغل لمنع التداخل
    if (video.hls) {
        video.hls.destroy();
    }

    runHls(video, primary);

    let isPlayed = false;
    video.onplaying = function() { 
        isPlayed = true; 
        btn.innerHTML = '<i class="fas fa-check-circle"></i> <span>البث المباشر يعمل</span>';
        setTimeout(() => { btn.innerHTML = originalText; btn.style.pointerEvents = 'auto'; }, 2000);
    };

    // مهلة الفحص: إذا لم تعمل القناة خلال 8 ثوانٍ، يتم التحويل للاحتياطي
    setTimeout(() => {
        if (!isPlayed) {
            console.log("التحويل للبث الاحتياطي للقناة: " + videoId);
