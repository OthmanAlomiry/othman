...
<head>
    ...
    <style>
        ...
        .play-btn-premium:hover { background: var(--main); transform: translateY(-3px); box-shadow: 0 10px 25px rgba(225, 29, 72, 0.5); border-color: #fff; }
    ...
    <div class="card">
        <div class="c-head">
            <div class="name-box-green">
                <span class="channel-name"> STARZPLAY <?php echo ($i-9); ?></span>
            </div>
            <div class="live-status-box">
                <div class="live-dot"></div>
                <span class="live-text">Live Stream</span>
...
/**
 * دالة التشغيل الذكية المحدثة
 * قمنا بإضافة 'this' كباراميتر لضمان استهداف الزر الصحيح في القنوات 6-9
 */
function smartPlay(videoId, primary, backup, btnElement) {
    const video = document.getElementById(videoId);
    const originalText = btnElement.innerHTML;
    
    btnElement.innerHTML = '<i class="fas fa-spinner fa-spin"></i> <span>جاري فحص البث...</span>';
    btnElement.style.pointerEvents = 'none';

    // محاولة جلب الرابط من الملف أولاً للتأكد من المحتوى
    fetch(primary)
        ...
        .then(streamUrl => {
            ...
            if (url.startsWith('http')) {
                ...
                video.onplaying = function() { 
                    isPlayed = true; 
                    btnElement.innerHTML = '<i class="fas fa-check-circle"></i> <span>البث الأساسي يعمل</span>';
                    setTimeout(() => { btnElement.innerHTML = originalText; btnElement.style.pointerEvents = 'auto'; }, 2000);
                };

                // مهلة فحص التشغيل الفعلي
                setTimeout(() => {
                    if (!isPlayed) {
                        switchToBackup(video, backup, btnElement, originalText);
                    }
                }, 8000);
            } else {
                switchToBackup(video, backup, btnElement, originalText);
...
function switchToBackup(video, backupUrl, btnElement, originalText) {
    btnElement.innerHTML = '<i class="fas fa-shield-alt"></i> <span>تفعيل الاحتياطي...</span>';
    fetch(backupUrl)
        .then(r => r.text())
        .then(url => {
            runHls(video, url.trim());
            setTimeout(() => { btnElement.innerHTML = originalText; btnElement.style.pointerEvents = 'auto'; }, 3000);
        })
        .catch(() => {
            btnElement.innerHTML = '<i class="fas fa-times"></i> <span>عذراً، البث متوقف</span>';
            setTimeout(() => { btnElement.innerHTML = originalText; btnElement.style.pointerEvents = 'auto'; }, 3000);
        });
}
