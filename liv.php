...
<div class="grid">
    <div id="section-alkas" class="channel-section">
        <div class="card">
            ...
            <div class="video-container" id="container_sh">
                <video id="vsh" playsinline controls></video>
            </div>
            <button class="play-btn" onclick="playShoof('vsh', 'sh1.php', this)">تشغيل بث قناة شوف</button>
        </div>
        <?php for($i=1; $i<=7; $i++): ?>
        <div class="card">
            ...
            <button class="play-btn" onclick="robustPlay('vkas<?php echo $i; ?>', 'k<?php echo $i; ?>.php', this)">بدء البث</button>
        </div>
        <?php endfor; ?>
    </div>
    ...
    <div id="section-bein" class="channel-section active">
        ...
        <div class="card">
            ...
            <button class="play-btn" onclick="robustPlay('vid<?php echo $i; ?>', 'b<?php echo $i; ?>.php', this)">بدء البث</button>
        </div>
        <?php endfor; ?>
    </div>
</div>
...
// دالة خاصة لقناة شوف لتجاوز الحماية
function playShoof(vId, p, btn) {
    const video = document.getElementById(vId);
    btn.innerText = "جاري فتح البث...";
    
    // محاولة التشغيل عبر HLS أولاً
    if (Hls.isSupported()) {
        const hls = new Hls();
        hls.loadSource(p);
        ...
        hls.on(Hls.Events.ERROR, function (event, data) {
            if (data.fatal) {
                // إذا فشل المشغل بسبب الحماية، نقوم بفتح الرابط في نافذة جديدة لضمان عمله للمستخدم
                window.open('sh1.php', '_blank');
                btn.innerText = "تم فتح البث في نافذة خارجية";
            }
        });
