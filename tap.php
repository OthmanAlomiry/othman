...
<head>
    ...
    <style>
        ...
        .cat-item { min-width: 70px; flex-shrink: 0; background: var(--glass); padding: 10px 5px; border-radius: 15px; cursor: pointer; text-align: center; font-size: 9px; font-weight: 900; }
        ...
        .match-card { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; padding: 15px; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; position: relative; }
        ...
        #loader { text-align: center; padding: 40px; opacity: 0.5; font-size: 12px; }
    </style>
</head>
<body>
...
<div class="main-container">
    <div class="header-fixed">
        <div class="social-links" style="display:flex; justify-content:space-around; margin-bottom:15px;">
            <a href="https://wa.me/966505571164" style="background:#25d366; color:white; padding:5px 15px; border-radius:20px; font-size:10px; text-decoration:none;">واتساب</a>
            <a href="https://t.me/d_s_pro" style="background:#0088cc; color:white; padding:5px 15px; border-radius:20px; font-size:10px; text-decoration:none;">تليجرام</a>
        </div>
    ...
    <div class="grid">
        ...
        <div id="section-<?= $s['key'] ?>" class="channel-section">
            ...
            <div class="card">
                ...
                <button class="play-btn" onclick="startStream('vid-<?= $ch['id'] ?>', '<?= $ch['file'] ?>')">تشغيل البث</button>
            </div>
            <?php endforeach; ?>
        </div>
        <?php endforeach; ?>
...
async function fetchMatches() {
    try {
        ...
        document.getElementById('matches-list').innerHTML = '<div style="text-align:center; padding:30px;">فشل الاتصال ببيانات المباريات.</div>';
    }
}
...
function renderMatches(matches) {
    ...
    if (!matches || matches.length === 0) {
        list.innerHTML = '<div style="text-align:center; padding:30px;">لا توجد مباريات متاحة اليوم.</div>';
        return;
    }
    list.innerHTML = '<div style="margin-bottom:20px; font-weight:900; color:var(--sky);">مباريات اليوم الهامة</div>';
    
    // فلترة الدوريات الهامة عثمان
    const important = [307, 2, 3, 5, 39, 140, 135, 78, 61];
        ...
        list.innerHTML += `
            <div class="match-card">
                ...
                <div class="m-info">
                    ...
                    <div class="m-time">${m.fixture.status.short === 'NS' ? time : 'مباشر'}</div>
                </div>
