...
ini_set('display_errors', 0); // إخفاء الأخطاء عن الزوار عثمان

// --- إعدادات السحابة و API المباريات عثمان ---
...
// --- دالة جلب بيانات القنوات عثمان ---
function getMyCloudData($bin, $key) {
...
$cloud = getMyCloudData($BIN_ID, $API_KEY_BIN);
...
function filterSectionMatches($channels, $sec) {
...
<head>
    ...
    <style>
        ...
        body { margin: 0; font-family: 'Tajawal', sans-serif; background: var(--bg); padding-top: 220px; color: #fff; display: flex; justify-content: center; overflow-x: hidden; }
        ...
        .m-score { font-size: 22px; font-weight: 900; font-family: sans-serif; }
...
<div class="main-container">
    ...
    <div class="grid">
        <div id="section-matches" class="channel-section active">
            <div id="matches-list">
                <div style="text-align:center; padding:50px; opacity:0.5;">جاري تحديث مباريات اليوم...</div>
            </div>
        </div>

        <?php foreach($active_sections as $s): $channels = filterSectionMatches($all_channels, $s['key']); ?>
...
async function loadMatchesData() {
    ...
    try {
        ...
        if (matches.length === 0) {
            // محاولة جلب المباريات القادمة إذا كان اليوم فارغاً عثمان
            const nextRes = await fetch(`https://v3.football.api-sports.io/fixtures?next=10&timezone=Asia/Riyadh`, {
                headers: { 'x-apisports-key': FOOTBALL_KEY }
            });
        ...
        if (matches.length === 0) {
            list.innerHTML = '<div style="text-align:center; padding:50px; opacity:0.5;">لا توجد مباريات حالياً.</div>';
            return;
        }

        list.innerHTML = '<div style="margin-bottom:15px; font-weight:900; color:var(--sky); padding-right:10px; border-right:3px solid var(--sky);">جدول المباريات</div>';
        ...
        list.innerHTML = '<div style="text-align:center; padding:50px;">تأكد من اتصال الإنترنت أو مفتاح API.</div>';
    }
}
